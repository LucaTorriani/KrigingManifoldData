...
private:
  ...
  const std::shared_ptr<const MatrixXd> _design_matrix;
  ...
  MatrixXd _residuals;
public:
  Model(const std::shared_ptr<const MatrixXd> data_tspace, const std::shared_ptr<const MatrixXd> design_matrix, unsigned int p): _data_tspace(data_tspace), _design_matrix(design_matrix),
      _N(_data_tspace->rows()), _p(p), _num_cov(_design_matrix->cols()), _num_coeff((_p*(_p+1))/2), _beta_matrix(_num_cov, _num_coeff){};
