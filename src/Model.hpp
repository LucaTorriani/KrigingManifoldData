...
private:
  ...
  /*! Number of significant entries in a symmetric _p*_p matrix.  \f$ \text{\_num\_coeff} = \frac{\text{\_p}*\left( \text{\_p}+1\right)}{2}  \f$ */
  ...
  /*! \f$ \left(\text{N\_tot}*\text{\_num\_coeff}\right) \f$ matrix where the \f$i^th\f$ row contains the upper triangular part of the matrix fitted in the \f$i^th\f$ location on the tangent space by the linear model. \f$ \text{\_fitted\_values} = \left(*\left(\text{\_design\_matrix\_tot}\right)\right)*\text{\_beta\_matrix} \f$ */
  MatrixXd _fitted_values;
  /*! \f$ \left(\text{N\_tot}*\text{\_num\_coeff}\right) \f$ matrix where the \f$i^th\f$ row contains the upper triangular part of the residual matrix in the \f$i^th\f$ location.  \f$  \text{\_residuals} = \left(*\left(\text{\_data\_tspace}\right)\right)-\text{\_fitted\_values} \f$  */
  MatrixXd _residuals;
public:
  /*!
    @brief Constructor
  ...
  */
  Model(const std::shared_ptr<const MatrixXd> data_tspace, const std::shared_ptr<const MatrixXd> design_matrix_model,  const std::shared_ptr<const MatrixXd> design_matrix_tot, unsigned int p, const std::string& distance_Manifold_name): // KERNEL
        ...
        _N(design_matrix_model->rows()), _p(p), _num_cov(_design_matrix_model->cols()), _num_coeff((_p*(_p+1))/2), _beta_matrix(_num_cov, _num_coeff){};
  /*!
    @brief Update \f$\texttt{\_beta\_matrix}\f$, \f$\texttt{\_fitted\_values}\f$ and \f$\texttt{\_residuals}\f$ according to the new covariogram matrix
    @param gamma_matrix Covariogram matrix
  */
  void update_model(const MatrixXd& gamma_matrix);
  /*!
    @brief Return \f$\texttt{\_beta\_matrix}\f$
  */
  MatrixXd get_beta() const;
  /*!
    @brief Return \f$\texttt{\_residuals}\f$
  */
  MatrixXd get_residuals() const;
  /*!
    @brief Return \f$\texttt{\_fitted\_values}\f$
  */
