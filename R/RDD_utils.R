...
RDD_OOK_boot_man = function(data, K, grid, nk_min, B,
                            # spdist,
                            ...
                            vario_model, distance) # *NEW*
{
  ...
  for(b in 1:B) # Repeat over the B bootstrap replicates
  {
    ...
    ### ------------- 1. Create RDD
    rdd = create.rdd(K=K, method.rdd = 'Voronoi', data = data,
      ...
      if(nk[as.character(k)]>0)  #table(factor(assigng, levels=1:K))[as.character(k)]>0
      {
        ...
        if(method.analysis == 'Kriging') {
          ...
          model.man = model_kriging(datamat, coords = datak[,c(1,2)],
                                    ...
                                    data_dist_mat=graph.distance[which(assign==k),which(assign==k)], data_grid_dist_mat=data.grid.distance[which(assign==k),which(assigng==k)],
